...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  // Đoàn này lấy activeBoard từ redux ra để mục đích là lấy được toàn bộ thông tin những thành viên của cái board thông qua field: FE_allUsers
  ...
  const handleUpdateCardMembers = (user) => {
    // console.log(user)
    // Tạo một biến incommingMemberInfo để gửi cho BE, với 2 thông tin chính là userId và action
    ...
    // console.log('incommingMemberInfo', incommingMemberInfo)
    ...
    <Box sx={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
      ...
      {FE_CardMembers.map((user, index) =>
        <Tooltip title={user.displayName} key={index}>
          <Avatar
            sx={{ width: 34, height: 34, cursor: 'pointer', ml: '-8px', border: '2px solid #a8a2e8ff'
            }}
